...
# Pro zabezpečení metody proti Cross-Site Request Forgery (XSRF) útokům je nutné do procesu přidat token, který bude ověřovat, že požadavek pochází od oprávněného uživatele. 
# Tento token by měl být součástí uživatelské session a měl by být ověřován při každé požadavku, který může ovlivnit stav na serveru. Zde je příklad, jak by se metoda mohla upravit:
...
#       specials: Other special values for this request.
#       params: Cgi parameters.
#       session: The session object containing the XSRF token.
#     """
#     # Verify XSRF token
#     if not self._VerifyXSRFToken(params, session):
...
# def _VerifyXSRFToken(self, params, session):
#     """Verifies if the XSRF token in the parameters matches the one in the session.

#     Args:
#       params: Cgi parameters containing the XSRF token submitted by the form.
#       session: The session object containing the XSRF token to verify against.

#     Returns:
#       True if the token matches and is valid, False otherwise.
#     """
#     token_param = self._GetParameter(params, 'xsrf_token')
#     token_session = session.get('xsrf_token')
#     return token_param and token_session and token_param == token_session
